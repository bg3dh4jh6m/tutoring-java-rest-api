...
import info.nenkov.warehouse.dao.RevisionRepository;
...
public class RevisionRestControllerIntegrationTest implements RepositoryIntegrationTest {
  ...
  public void setupStocksAndOrdersForRevision() {
    ...
    revisionRepository.insertNewRevision(
        900,
  ...
  public void createNewRevisionWithTimeInterval() {
    assertThat(
            this.restTemplate.getForObject(
                "http://localhost:" + port + "/revision/interval?from=2012-01-01&to=2020-01-01",
                String.class))
        .isEqualTo(
            "{"
                + "\"revisionId\":4,"
  ...
  public void checkTheSumOfAllRevisionsInTheLastThirtyMinutes() {
    assertThat(
            this.restTemplate.getForObject(
                "http://localhost:" + port + "revision/last", String.class))
        .contains(
            "{\"revisionId\":4,"
