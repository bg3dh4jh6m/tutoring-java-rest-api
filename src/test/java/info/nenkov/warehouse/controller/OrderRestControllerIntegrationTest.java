...
public class OrderRestControllerIntegrationTest implements RepositoryIntegrationTest {
  ...
  public void prepare() {
    ...
    jdbcTemplate.execute(
        "INSERT INTO `stock` (`id`,`stock_id`,`quantity`,`price`) " + "VALUES (16,442,500,1.25);");
    jdbcTemplate.execute(
        "INSERT INTO `order` (`order_id`,`stock_id`,`quantity`,`price`,`order_timestamp`) "
  ...
  public void getOrderById() {
    assertThat(
            this.restTemplate.getForObject(
                "http://localhost:" + port + "/order/show/19", String.class))
        .isEqualTo(
            "{\"orderId\":19,"
                + "\"quantity\":20,"
                + "\"price\":10.05,"
  ...
  public void createNewOrder() {
    ...
    assertThat(
            this.restTemplate.postForObject(
                "http://localhost:" + port + "/order/new", map, String.class))
        .contains(
            "{"
                + "\"orderId\":20,"
                + "\"quantity\":10,"
  ...
  public void failToCreateNewOrderOfNonExistingStock() {
    ...
    assertThat(
            this.restTemplate.postForObject(
                "http://localhost:" + port + "/order/new", map, String.class))
        .contains("Could not find s stock with id=999999999");
  }
